...
if __name__ == "__main__":
    ...
    for idx, row in df.iterrows():
        ...
        llmcols = [x for x in df.columns if "_lm" in x]
    ...
    meta_df = pd.read_csv('data/clean/qualtrics_loop_merge_completion_annot_output_random_full.csv')
    df = pd.merge(df, meta_df, how='left', left_on=['loop_merge'], right_on=['idx'])

    df['correct'] = (df['answer'].str.strip() == df['choice_opt_str'].str.strip()).astype(int)

    print(array_stats(df['correct'].values, include_ci=
        True))
    print()

    print(categorical_stats(df['correct'].values))

    print()
    k = len(df.query("correct==1"))
    n = len(df)
    p = 0.5
