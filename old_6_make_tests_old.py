...
- test_uid: UID of the test example. {context}_{correlation}_

In other words, for the same test, the test_id will be the same (since it consists of the same training examples), but the test_uid will be different for each question.
So test_uid is unique to each test question, while test_id is unique to the "test configuration" which I am defining as (context, v1, s1, v2, s2, n_train).
